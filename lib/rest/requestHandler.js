...
class RequestHandler {
    lastRequestTime; // Tracks the time of the last request.
    requestInterval; // Minimum interval between requests in ms.
    requestCount; // Counter for requests sent.
    ...
    async request(method, endpoint, auth = true, data, reason, files, headers) {
        try {
            const response = await this.fetchFromAPI(method, endpoint, auth, { data }, reason, files, headers);
     ...
     */
    async fetchFromAPI(method, url, auth = true, body, reason, files, addHeaders) {
            ...
            bucket.addRequest(async () => {
                ...
                try {
                    ...
                    if (error instanceof RestErrors_1.RateLimitError) {
                        ...
                        resolve(await this.fetchFromAPI(method, url, auth, body, reason, files)); // Retry on rate limit.
                    }
                    else {
                        reject(error);
                    }
