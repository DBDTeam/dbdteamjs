...
class RequestHandler {
    ...
    async request(method, url, auth = true, body, reason, files, force) {
        ...
        try {
            ...
            const response = await this.makeResponse(finalURL, options, method, headers, body, files, force);
            return response;
        }
        catch (error) {
    ...
    makeResponse(finalURL, options, method, headers, body, files, force) {
        const a = Date.now();
