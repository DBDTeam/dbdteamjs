...
class CDN {
    ...
    discoverySplash(guild, options) {
        const data = this._getInfo(options, EXTENSIONS.NOTGIF, guild?.discovery_splash);
        return this.BASE_URL(`/discovery-splashes/${guild?.id}/${guild?.discovery_splash}.${data.extension}?size=${data.size}`);
    }
    /**
