...
class CDN {
     ...
     */
    avatar(id, avatar, options = {}) {
        const data = this._getInfo(options, EXTENSIONS.GLOBAL, avatar);
        return this.BASE_URL(`/avatars/${id}/${avatar}.${data.extension}?size=${data.size}`);
    }
    /**
     ...
     */
    defaultAvatar(discriminator, id) {
     ...
     */
    banner(id, banner, options) {
        const data = this._getInfo(options, EXTENSIONS.GLOBAL, banner);
        return this.BASE_URL(`/banners/${id}/${banner}.${data.extension}?size=${data.size}`);
    }
    /**
     ...
     */
    avatarDecoration(id, decoration, options) {
        const data = this._getInfo(options, EXTENSIONS.NOTGIF, decoration);
        return this.BASE_URL(`/avatars-decoration/${id}/${decoration}.${data.extension}?size=${data.size}`);
    }
    // EMOJIS
    /**
     ...
     */
    guildIcon(id, icon, options) {
        const data = this._getInfo(options, EXTENSIONS.GLOBAL, icon);
        return this.BASE_URL(`/icons/${id}/${icon}.${data.extension}`);
    }
    /**
     ...
     */
    guildSplash(id, splash, options) {
        const data = this._getInfo(options, EXTENSIONS.NOTGIF, splash);
        return this.BASE_URL(`/splashs/${id}/${splash}.${data.extension}?size=${data.size}`);
    }
    /**
     ...
     */
    discoverySplash(id, discovery_splash, options) {
        const data = this._getInfo(options, EXTENSIONS.NOTGIF, discovery_splash);
        return this.BASE_URL(`/discovery-splashes/${id}/${discovery_splash}.${data.extension}?size=${data.size}`);
    }
    /**
     ...
     */
    roleIcon(roleIcon, options) {
        const data = this._getInfo(options, EXTENSIONS.NOTGIF, roleIcon);
        return this.BASE_URL(`/role-icons/${roleIcon}/${roleIcon}.${data.extension}?size=${data.size}`);
    }
    /**
    ...
    _getInfo(opts, exten, d) {
        ...
        var _opts = utils_1.Utilities.setObj(OPTIONS, opts);
        if (_opts.size)
