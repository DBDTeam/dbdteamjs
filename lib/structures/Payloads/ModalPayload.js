...
class InteractionModalPayload {
     ...
     */
    constructor(data) {
        this.#d = utils_1.Utilities.setObj(this.#DATA, data, { custom_id: "customId" });
        for (var i in this.#d?.components) {
            const d = this.#d.components[i];
            for (var x in d?.components) {
                var _data = utils_1.Utilities.setObj(this.#COMPONENT_DATA, this.#d.components[i]?.components[x], {
                    custom_id: "customId",
