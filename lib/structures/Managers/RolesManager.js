...
class GuildRolesManager {
    ...
    async fetch(roleId) {
        ...
        if (!response || response.error)
            return response;
        return new Role_1.GuildRole(response, this.guild, this.#client);
    }
    /**
    ...
    async edit(id, editOptions) {
        ...
        if (response.error)
            return response;
        let role = new Role_1.GuildRole(response, this.guild, this.#client);
        this.cache.set(role.id, role);
        return role;
    }
    /**
    ...
    async delete(deleteObject) {
        ...
        for (var i in roles) {
            ...
            if (response.error)
                errors.push(response);
            const role = new Role_1.GuildRole(response, this.guild, this.#client);
            success.push(role);
        }
        return { error: errors, success };
    }
    ...
    async create(createObject) {
        ...
        if (response.error)
            return response;
        const role = new Role_1.GuildRole(response, this.guild.id, this.#client);
