...
class ThreadMemberManager {
     ...
     * @private
     */
    async #fetchAllMembersInThread(config) {
        ...
        for (var member of response.data) {
            var thread_member = new ThreadMember_1.ThreadMember(member, this.guild, this.#client);
    ...
    async fetch(memberId) {
        if (typeof memberId === "string") {
            ...
            if (!result || result?.hasData())
                return result;
            var thread_member = new ThreadMember_1.ThreadMember(result.data, this.guild, this.#client);
            this.cache.set(thread_member.id, thread_member);
    ...
    async remove(memberId) {
        const response = await this.#client.rest.request("DELETE", Endpoints.ChannelThreadMember(this.id, memberId));
        if (response?.error)
            return false;
