...
class ApplicationCommandManager {
    ...
    async add(body) {
        ...
        if (this.target !== "global") {
            var response = await this.#client.rest.request("POST", Endpoints.ApplicationGuildCommands(this.#client.user.id, this.target), true, { data: body });
        }
        else {
            var response = await this.#client.rest.request("POST", Endpoints.ApplicationCommands(this.#client.user.id), true, { data: body });
        }
        if (!response)
            return null;
        if (response.error) {
    ...
    async set(commands) {
        ...
        if (!this.#client.user)
            return;
        if (this.target !== "global") {
            var response = await this.#client.rest.request("PUT", Endpoints.ApplicationGuildCommands(this.#client.user.id, this.target), true, { data: commands });
        }
        else {
            var response = await this.#client.rest.request("PUT", Endpoints.ApplicationCommands(this.#client.user.id), true, { data: commands });
        }
        if (!response)
            return null;
        if (response.error) {
