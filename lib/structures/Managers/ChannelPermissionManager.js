...
const Permissions_1 = require("../../common/interfaces/general/Permissions");
const Endpoints = __importStar(require("../../rest/Endpoints"));
const Permission_1 = require("../Flags/Permission");
...
class ChannelPermissionManager {
    ...
    resolvePermissions(permsObj) {
        ...
        if (permsObj.allow) {
            ...
            for (const perm of allowArray) {
                if (!Permission_1.PermissionManager.isValidPermission(perm))
     ...
     */
    getPermissionBitfields(target) {
        const preparedTarget = this.prepareTarget(target);
        const permissionOverwrites = this.channel?.permission_overwrites || [];
        const targetOverwrites = permissionOverwrites.find((overwrite) => overwrite.id === preparedTarget.targetId);
        var allow = BigInt(0);
        var deny = BigInt(0);
        if (targetOverwrites) {
        ...
        if (preparedTarget.type === "1") {
            ...
            if (target.target?.role_ids) {
                for (const roleId of target.target?.role_ids) {
                    const founded = permissionOverwrites.find((overwrite) => overwrite.id === roleId);
                    if (!founded)
                        continue;
     ...
     */
    async has(target, permissions, forceAdmin = false, matchAll = true) {
        ...
        if (typeof permissions === "object" && !Array.isArray(permissions)) {
            ...
            const conditions = Object.entries(permissions).map(([permission, required]) => {
                const permissionBit = BigInt(this.#permissionsBits[permission]);
                if (!permissionBit) {
     ...
     */
    async hasAny(target, permissions) {
        if (!permissions || !["string", "object"].includes(typeof permissions))
            throw new ClientError_1.ClientError(ErrorList_1.ErrorNames.InvalidType, "string | object", "permissions");
        if (typeof permissions === "string") {
            permissions = [permissions];
