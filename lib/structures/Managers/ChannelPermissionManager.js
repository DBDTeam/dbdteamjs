...
class ChannelPermissionManager {
     ...
     */
    async edit(TargetPayload, permsObj, reason = null) {
        let addedPerms = 0;
        let removedPerms = 0;
        const obj = getType(TargetPayload == "everyone" ? this.target.guildId : TargetPayload, this.target);
        if (!permsObj)
            return null;
     ...
     * @returns {Promise<ErrorResponseFromApi | ChannelPermissionSuccessResponse | null>} - The response from the API.
     */
    async add(TargetPayload, permsObj, reason = null) {
        const obj = getType(TargetPayload == "everyone" ? this.target.guildId : TargetPayload, this.target);
        const result = this.#resolve(permsObj);
     ...
     * @returns {Promise<ErrorResponseFromApi | ChannelPermissionSuccessResponse | null>} - The response from the API.
     */
    async remove(TargetPayload, permsObj, reason = null) {
        const obj = getType(TargetPayload == "everyone" ? this.target.guildId : TargetPayload, this.target);
        const result = this.#resolve(permsObj);
