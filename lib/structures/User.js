...
class User extends Base_1.Base {
    ...
    _patch(data) {
        ...
        this.badges = Object.entries(common_1.BadgesBitfieldValues)
    ...
    async createDM() {
        ...
        const result = await this.#client.rest.request("POST", Endpoints.UserDM(), true, { recipient_id: this.id });
        if (result?.error || !result?.data)
            return result;
        const dm = utils_1.Utilities.typeChannel(result.data, this.#client);
        this.dmChannel = dm;
        return dm;
    }
    async send(body) {
