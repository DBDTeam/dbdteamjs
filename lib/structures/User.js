...
class User extends Base_1.Base {
    ...
    async createDM() {
        ...
        const result = await this.#client.rest.request("POST", Endpoints.UserDM(), true, { data: { recipient_id: this.id } });
        if (result?.error || !result?.data)
            return result;
        const dm = (0, utils_1.typeChannel)(result.data, this.#client); // idk why, but it works, if
