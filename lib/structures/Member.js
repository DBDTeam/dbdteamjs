...
class Member extends Base_1.Base {
    ...
    constructor(data, guild, client) {
        ...
        this.joined = utils_1.Utilities.getAllStamps(this);
    ...
    _patch(data) {
        ...
        if ("premium_since" in data &&
            ...
            this.premiumSince = utils_1.Utilities.getAllStamps(this.#PREMIUM);
        }
        if ("pending" in data) {
        ...
        if ("communication_disabled_until" in data) {
            this.communicationDisabledUntil = utils_1.Utilities.getAllStamps(this.#TIMEOUTED);
    ...
    async edit(editPayload) {
        ...
        var response = await this.#client.rest.request("PATCH", Endpoints.GuildMember(this.guild.id, this.id), true, payload.payload, reason);
    ...
    async changeNickname(nickname, reason) {
        ...
        reason = reason?.trim();
        var response = await this.#client.rest.request("PATCH", Endpoints.GuildMember(this.guild.id, this.id), true, { roles: this.roles, flags: this.flags, nick: nickname }, reason);
        return response?.error ? false : true;
    }
    /**
