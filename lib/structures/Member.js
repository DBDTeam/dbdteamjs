...
class Member extends Base_1.Base {
    ...
    _patch(data) {
        ...
        this.permissions = new MemberPermissionManager_1.MemberPermissionManager(this.#client, this, this.guild);
        if (this.id === this.#client.user.id) {
            this.edit;
            this.kick;
            this.ban;
    ...
    get kickable() {
        ...
        const conditions = {
            kick: clientPermissions &
                (PermissionsBitFields_1.PermissionsBitField.Roles.KickMembers |
                    PermissionsBitFields_1.PermissionsBitField.Roles.Administrator),
    ...
    get bannable() {
        ...
        const clientPermissions = clientMember?.roles.cache
            ...
            .reduce((permissions, role) => permissions | role.permissions, 0) || 0;
        const conditions = {
            ban: clientPermissions &
                (PermissionsBitFields_1.PermissionsBitField.Roles.BanMembers |
                    PermissionsBitFields_1.PermissionsBitField.Roles.Administrator),
            client: this.id !== this.#client.user.id,
            owner: this.id.toString() !== this.guild.owner_id,
            highest: highestRolePosition <= clientHighestRolePosition,
