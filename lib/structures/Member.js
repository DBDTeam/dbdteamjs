...
class Member extends Base {
    ...
    get kickable() {
        var _p = 0;
        var _h = this.roles.cache.toJSON().sort((a, b) => b.position - a.position)?.[0]
            ?.position || 0;
        const c = this.guild.members.me;
        var _h1 = c.roles.cache.toJSON().sort((a, b) => b.position - a.position)?.[0]
            ?.position || 0;
        for (var perms of c.roles.cache.toJSON()) {
            _p |= perms.permissions;
        }
        const conditions = {
            kick: _p & PermissionsBitField.Roles.KickMembers ||
    ...
    get banneable() {
        ...
        const conditions = {
            ban: _p & PermissionsBitField.Roles.BanMembers ||
                _p & PermissionsBitField.Roles.Administrator,
            client: this.id !== this.#client.user.id,
            owner: this.id.toString() !== this.guild.ownerId,
            highest: _h <= _h1,
