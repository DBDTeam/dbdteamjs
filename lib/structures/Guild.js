...
class Guild extends Base_1.Base {
    ...
    constructor(data, client) {
         ...
         * @type {object}
         */
        this.created = utils_1.Utilities.getAllStamps(this);
        /**
    ...
    #patch() {
        if ("channels" in this.#data) {
            for (var channelData of this.#data.channels) {
                const channel = utils_1.Utilities.typeChannel(channelData, this.client);
    ...
    async edit(body) {
        if (!body || typeof body !== "object")
            return null;
        const response = await this.client.rest.request("PATCH", Endpoints.Guild(this.id), true, body);
        if (!response || response?.error)
            return response;
        return new Guild(response, this.client);
    }
}
exports.Guild = Guild;
