...
class InteractionBase {
    ...
    constructor(data, client) {
        ...
        this.user = new User_1.User(data.member.user, this.client);
    ...
    async __makeReply(obj) {
        ...
        var response;
        var request = await this.client.rest.request("POST", Endpoints.Interaction(this.interactionId, this.token), true, data, null, data?.data?.files);
        if (obj.fetchResponse) {
            request = await this.client.rest.request("GET", Endpoints.InteractionOriginal(this.client.user.id, this.token), true);
            if (!request?.error)
                return null;
    ...
    async deferReply(ephemeral) {
        if (ephemeral && typeof ephemeral !== "boolean")
            throw new ClientError_1.ClientTypeError(ErrorList_1.ErrorNames.InvalidType, "boolean", "ephemeral");
        ephemeral = !!ephemeral;
        this.__makeReply({
    ...
    async editReply(body) {
        ...
        if (!body?.content &&
            ...
            !body.poll &&
            !body.sticker_ids)
            throw new ClientError_1.ClientError(ErrorList_1.ErrorNames.MissingRequiredProperties, "Interaction response", ["content", "files", "embeds", "poll", "sticker_ids"]);
        ...
        if (request?.error)
            return request;
        const message = new InteractionResponse_1.InteractionResponse(request, this.client);
        return message;
    }
    /**
