...
class InteractionResponse extends Message_1.Message {
    ...
    async editInteractionResponse(body) {
        ...
        const response = await this.client.rest.request("PATCH", Endpoints.InteractionOriginal(this.client.user.id, this.token), true, message.payload, null, message.files);
        if (!response)
            return null;
        if (response.error)
            return response;
