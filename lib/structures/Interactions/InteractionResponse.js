...
class InteractionResponse extends Message_1.Message {
    ...
    async editInteractionResponse(obj) {
        ...
        const { payload, files } = isString
            ...
            : new EditMessagePayload_1.EditMessagePayload(obj, obj.files);
        const response = await rest.request("PATCH", Endpoints.InteractionOriginal(user.id, this.token), true, { data: payload }, null, files);
        if (!response)
            return null;
        if (response.error)
            return response;
