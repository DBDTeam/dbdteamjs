...
class ComponentInteraction extends BaseInteraction_1.InteractionBase {
    ...
    constructor(data, client) {
        ...
        this.update = (...args) => this.updateReply(args);
        this._patch();
    }
    /**
    ...
    async updateReply(obj) {
        ...
        if (obj.fetchReply) {
            response = await this.client.rest.request("GET", // @ts-ignore
            Endpoints.InteractionOriginal(this.client.user.id, this.token), true);
            if (!response)
                return null;
            // @ts-ignore
