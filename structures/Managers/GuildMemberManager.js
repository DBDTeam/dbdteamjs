...
class GuildMemberManager {
    ...
    async _fetchAllMembers(obj) {
        ...
        if (response.error) { return null } else {
            for (var m of response.data) {
                ...
                this.cache.set(x.id, new Member(x, this.#client.guilds.cache.get(this.guildId), this.#client))
            }
    ...
    async fetch(memberId) {
        const { Member } = require("../Member") // This is to avoid circular dependencies (idk why it happens)
        if (typeof memberId === "string") {
            ...
            if (result?.error) {
                ...
                var m = new Member(x, this.#client.guilds.cache.get(this.guildId) || this.guild, this.#client)
